...
export class DeFi {
    ...
    _setTokens(_tokens) {
        ...
        this.TXHToken = _tokens.find((_item) => _item.symbol === 'TXH');
    ...
    */
    ...
    canTokenBeDeposited(_token) {
        return _token && _token.isActive && _token.canDeposit && _token.symbol !== 'TXH';
    }
    ...
    canTokenBeTraded(_token) {
        ...
        return _token && _token.isActive && (_token.canTrade || _token.symbol === 'TXH');
     ...
     * @return {Promise<Number>}
     */
    async fetchTXHTokenPrice(_to = 'USD') {
        const data = await this.apolloClient.query({
            query: gql`
