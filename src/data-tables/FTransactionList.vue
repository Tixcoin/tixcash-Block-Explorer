...
-->
                ...
                <!--
                            <template v-slot:column-fee="{ value, column }">
                                <div v-if="column" class="row no-collapse no-vert-col-padding">
                                    <div class="col-5 f-row-label">{{ column.label }}</div>
                                    <div class="col">{{ WEIToTXH(value | formatHexToInt) }}</div>
                                </div>
                                <template v-else>
                                    {{ WEIToTXH(value | formatHexToInt) }}
                                </template>
                            </template>
                -->
...
*/
                    ...
                    {
                        name: 'amount',
                        label: `${this.$t('view_transaction_list.amount')} (TXH)`,
                        itemProp: `${!this.withoutCursor ? 'transaction.' : ''}value`,
                        formatter: _value => {
                            // return formatNumberByLocale(numToFixed(WEIToTXH(_value), 2), 2)
                            return WEIToTXH(_value);
                        },
                        width: '150px',
                        css: {
                            textAlign: 'right'
...
/*
                    {
                        name: 'fee',
                        label: `${this.$t('view_transaction_list.fee')} (TXH)`,
                        itemProp: 'gasUsed',
                        width: '130px'
                    }
*/
